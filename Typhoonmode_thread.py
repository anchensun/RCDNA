...
class Typhoon:
    ...
    @staticmethod
    def solve_pyramid(im, factrset = 1000, epsilonset = None, maxfunset = 60, maxiterset = 60, levels_pyr=1, solve_fast=False,
        ...
        im0 = im[0]
        im1 = im[1]
        if not(hasattr(im0, 'shape') and hasattr(im1, 'shape')):
            return (numpy.zeros([10, 10]), numpy.zeros([10, 10]))
        pyr_im0 = [im0,]
        pyr_im1 = [im1,]
        ...
        for l, (im0, im1) in enumerate(zip(pyr_im0, pyr_im1)):
            ...
            if solve_fast:
                ...
                U = typhoon.solve(im0, im1, U0=U0, wav=wav,
                                  ...
                                  levels_estim=levels_estim,
                                  factrset = factrset,
                                  epsilonset = epsilonset,
                                  maxfunset = maxfunset,
                                  maxiterset = maxiterset,
                                  **kwargs)
